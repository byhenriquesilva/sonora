...
fun GenreDetailScreen(
    ...
    val genreSongs = remember(songs, genre) {
        songs.filter {
    ...
    val totalDurationMs = remember(genreSongs) { genreSongs.sumOf { it.durationMs } }
    val totalMinutes = totalDurationMs / (1000 * 60)

    Scaffold(
        modifier = modifier.testTag("genre_detail_screen"),
        topBar = {
            TopAppBar(
        ...
        LazyColumn(
            ...
            item {
                Column(
                    ...
                    Text(
                        text = "${genreSongs.size} músicas • $totalMinutes min",
            ...
            itemsIndexed(genreSongs) { index, song ->
                SongListItem(
                    song = song,
                    isPlaying = isPlaying,
